...
class Runtime:
    ...
    token = ""
    loopback_url = ""
    config = {"base_url": "", "model": "claude-sonnet-4-20250514", "vision_model": "gemini-2.5-flash"}
...
def _new_run(kind: str, request_data: dict[str, Any]) -> tuple[str, dict[str, Any]]:
    ...
    with Runtime.mutex:
        ...
        if len(Runtime.runs) > 100:
            completed = [key for key, value in Runtime.runs.items() if value["status"] in ("done", "error", "cancelled")]
...
def _run_agent(run: dict[str, Any], request: AgentRequest) -> None:
    acquired = Runtime.run_lock.acquire(blocking=False)
    if not acquired:
        run["status"] = "error"
        _emit(run, "error", message="同一项目已有修改任务正在运行。")
        return
    workspace = None
    ...
    try:
        run["status"] = "running"
        images, attachments = _images(request.attachment_ids)
        ...
        generator = GameGenerator(
            api_key=api_key,
            base_url=Runtime.config["base_url"] or None,
            model=Runtime.config["model"],
            history_file=str(_history_path()),
            runner=GodotRunner(Runtime.godot_exe or None),
            cancel_event=run["cancel"],
            history_key=str(Runtime.project),
        )
        ...
        def step_callback(step) -> None:
            ...
            if changed:
                all_changed.extend(name for name in changed if name not in all_changed)
                _emit(run, "step", type="files_changed", description="已应用文件变更", changed_files=changed)
            value = step.to_dict()
            _emit(run, "step", **value)

        result = generator.agent_generate(request.prompt, str(workspace.stage), step_callback, images)
        changed = workspace.commit(request.dirty_files)
        all_changed.extend(name for name in changed if name not in all_changed)
        status = "cancelled" if run["cancel"].is_set() else "done"
        run["status"] = status
        _annotate_history(request.prompt, attachments, status)
        harness = AcceptanceHarness(Runtime.project, Runtime.godot_exe)
        plan = harness.load_plan()
        _emit(run, "acceptance_plan", plan=plan)
        _emit(run, "done", status=status, reply=result.get("reply", ""), changed_files=all_changed,
              validation_ok=bool(result.get("validation", {}).get("ok", result.get("ok", False))))
    except InterruptedError as error:
        run["status"] = "cancelled"
        _annotate_history(request.prompt, attachments, "cancelled")
        _emit(run, "done", status="cancelled", reply="", changed_files=all_changed, message=str(error))
    except (Conflict, ValueError) as error:
        run["status"] = "error"
        _emit(run, "error", message=str(error), retryable=True, changed_files=all_changed)
...
def _run_harness(run: dict[str, Any], request: HarnessRunRequest) -> None:
    acquired = Runtime.run_lock.acquire(blocking=False)
    if not acquired:
        run["status"] = "error"
    ...
    try:
        run["status"] = "running"
        harness = AcceptanceHarness(Runtime.project, Runtime.godot_exe)
        summary = harness.run(request.case_ids, lambda name, payload: _emit(run, name, **payload), run["cancel"])
        run["status"] = "cancelled" if summary["cancelled"] else "done"
        _emit(run, "done", status=run["status"], harness=summary)
    except Exception as error:
        run["status"] = "error"
        log.exception("Harness run failed")
        _emit(run, "error", message=str(error), retryable=True)
    finally:
        Runtime.run_lock.release()
